...
const GAMEOVER_COLOR:Color = [0.00, 0.00, 0.80, 0.50];
...
impl Game{
    ...
    fn add_food (&mut self){
        ...
        let mut coord_list: Vec<(i32, i32)> = Vec::new();
        ...
        println!("{}", coord_list.len());
        if coord_list.len() <= 0{
            self.game_over = true;
            return;
        }
        let index = rng.gen_range(0..coord_list.len());
        let coord = coord_list[index];
        self.food_x = coord.0;
        self.food_y = coord.1;
